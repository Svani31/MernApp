/* eslint-disable react-hooks/exhaustive-deps */
"use client";
import { getSession, signIn } from "next-auth/react";
import React, { use, useEffect, useState } from "react";
import { useRouter } from "next/navigation";

interface UserInfoProps {
  name: string;
  password: string;
  email?: string;
  image?: string;
}

function LandingPage() {
  const [userInfo, setUserInfo] = useState<UserInfoProps>({
    name: "",
    password: "",
    email: "",
    image:
      "data:image/png;base64,",
  });

  const [haveUserAccount, setHaveUserAccount] = useState<boolean>(false);

  const [loginInfo, setLoginInfo] = useState<UserInfoProps>({
    name: "",
    password: "",
  });

  const route = useRouter();


  // log in into user using prisma auth
  const submitHandler = async (e: any) => {
    e.preventDefault();
    const signInUser = await signIn("credentials", {
      redirect: false,
      name: loginInfo.name,
      password: loginInfo.password,
    });
    if (!signInUser?.ok) {
      alert("this is wrong");
    }
    console.log(signInUser, "this is user");
    if (signInUser?.status == 200) route.push("/components/main");
  };

  useEffect(() => {
    const fetchGetSession = async () => {
      const mySession = await getSession();
      if (mySession) route.push("/components/main");
    };
    fetchGetSession();
  }, []);


  // Create User 
  const createUserHandler = async (e:any) => {
    e.preventDefault()
    try{
      const createUser = await fetch(`${process.env.NEXT_PUBLIC_API_KEY}api/user`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          name:userInfo.name,
          email:userInfo.email,
          password:userInfo.password,
          image:userInfo.image,
        }),
      });
      console.log(createUser,"this is created User")
      if (createUser) route.push("/components/main");
    }catch(error){
      throw error
    }
  };

  return (
    <div className=" flex justify-around items-center mt-20 h-80 ">
      <div className=" max-w-96">
        <h1 className="font-bold text-3xl text-blue-700">
          Welcome on Messenger From Wish Dot Com
        </h1>
      </div>
      <div>
        {haveUserAccount ? (
          <div className="rounded-xl border-2 p-5 bg-blue-800">
            <h1 className="flex text-center justify-center font-bold text-white text-xl">
              Log In
            </h1>
            <div className="gap-2 flex ">
              <form className="p-5 gap-2 flex flex-col">
                <div className="border-2 rounded-xl p-1 mt-2 bg-white">
                  <input
                    type="text"
                    className="outline-none"
                    placeholder="Enter Your Name"
                    onChange={(e) =>
                      setLoginInfo({ ...loginInfo, name: e.target.value })
                    }
                  />
                </div>
                <div className="border-2 rounded-xl p-1 mt-2 bg-white">
                  <input
                    type="text"
                    className="outline-none"
                    placeholder="Enter Your Password"
                    onChange={(e) =>
                      setLoginInfo({ ...loginInfo, password: e.target.value })
                    }
                  />
                </div>
                <button
                  className="p-2 mt-4 bg-blue-500 rounded-xl flex justify-center items-center text-center font-bold text-white"
                  onClick={(e) => submitHandler(e)}
                >
                  Submit
                </button>
                <div
                  onClick={(e) => setHaveUserAccount(false)}
                  className="font-bold underline text-white"
                >
                  Dont Have an Account?
                </div>
              </form>
            </div>
          </div>
        ) : (
          <div className="rounded-xl border-2 p-5 bg-blue-800">
            <h1 className="flex text-center justify-center font-bold text-white text-xl">
              Registration
            </h1>
            <div className="gap-2 flex ">
              <form className="p-5 gap-2 flex flex-col">
                <div className="border-2 rounded-xl p-1 mt-2 bg-white">
                  <input
                    type="text"
                    className="outline-none"
                    onChange={(e) =>
                      setUserInfo({ ...userInfo, name: e.target.value })
                    }
                    placeholder="Enter Your Name"
                  />
                </div>
                <div className="border-2 rounded-xl p-1 mt-2 bg-white">
                  <input
                    type="text"
                    className="outline-none"
                    onChange={(e) =>
                      setUserInfo({ ...userInfo, email: e.target.value })
                    }
                    placeholder="Enter Your Email"
                  />
                </div>
                <div className="border-2 rounded-xl p-1 mt-2 bg-white">
                  <input
                    type="text"
                    className="outline-none"
                    onChange={(e) =>
                      setUserInfo({ ...userInfo, password: e.target.value })
                    }
                    placeholder="Enter Your Password"
                  />
                </div>
                <div className="border-2 rounded-xl p-1 mt-2 bg-white ">
                  <input
                    className="outline-none"
                    type="text"
                    placeholder="Confirm Your Password"
                  />
                </div>
                <button
                  onClick={(e) => createUserHandler(e)}
                  className="p-2 mt-4 bg-blue-500 rounded-xl flex justify-center items-center text-center font-bold text-white"
                >
                  Submit
                </button>
                <div
                  onClick={(e) => setHaveUserAccount(true)}
                  className="font-bold underline text-white"
                >
                  Have an Account?
                </div>
              </form>
            </div>
          </div>
        )}
      </div>
    </div>
  );
}

export default LandingPage;
