/* eslint-disable react-hooks/exhaustive-deps */
"use client";
import { getSession, signIn } from "next-auth/react";
import React, { use, useEffect, useState } from "react";
import { useRouter } from "next/navigation";

interface UserInfoProps {
  name: string;
  password: string;
  email: string;
  image?: string;
}

function LandingPage() {
  const [userInfo, setUserInfo] = useState<UserInfoProps>({
    name: "",
    password: "",
    email: "",
    image:"data:image/png;base64,",
  });

  const [haveUserAccount,setHaveUserAccount] = useState<boolean>(false)

  const [loginInfo,setLoginInfo] = useState<UserInfoProps>({
    name:"",
    password:"",
    email:""
  })

  const route = useRouter();

  const submitHandler = async (e: any) => {
    e.preventDefault();
    const signInUser = await signIn("credentials", {
      redirect: false,
      name: userInfo.name,
      email: userInfo.email,
      password: userInfo.password,
      image: userInfo.image,
    });
    if (!signInUser?.ok) {
      alert("this is wrong");
    }
    console.log(signInUser,"this is user")
    if (signInUser?.status == 200) route.push("/components/main");

  };

  useEffect(()=>{
    const fetchGetSession = async()=>{
        const mySession = await getSession()
        if(mySession) route.push("/components/main")
    }
    fetchGetSession()
},[])


  return (
    <div className=" flex justify-center items-center mt-16 ">
      {haveUserAccount ? (<div className="rounded-xl border-2 p-5 bg-blue-800">
        <h1 className="flex text-center justify-center font-bold text-white text-xl">Log In</h1>
        <div className="gap-2 flex ">
          <form className="p-5 gap-2 flex flex-col">
            <div className="border-2 rounded-xl p-1 mt-2 bg-white">
              <input
                type="text"
                className="outline-none"
                placeholder="Enter Your Name"
              />
            </div>
            <div className="border-2 rounded-xl p-1 mt-2 bg-white">
              <input
                type="text"
                className="outline-none"
                placeholder="Enter Your Password"
              />
            </div>
            <div className="border-2 rounded-xl p-1 mt-2 bg-white ">
              <input
                className="outline-none"
                type="text"
                placeholder="Confirm Your Password"
              />
            </div>
            <button
              className="p-2 bg-blue-500 rounded-xl flex justify-center items-center text-center font-bold text-white"
              onClick={(e)=> submitHandler(e)}
           >
              Submit
            </button>
            <div onClick={(e)=> setHaveUserAccount(false)}  className="font-bold underline text-blue-400">
              Dont Have an Account?
            </div>
          </form>
        </div>
      </div>) : ( <div className="rounded-xl border-2 p-5 bg-blue-800">
        <h1 className="flex text-center justify-center font-bold text-white text-xl">Registration</h1>
        <div className="gap-2 flex ">
          <form className="p-5 gap-2 flex flex-col">
            <div className="border-2 rounded-xl p-1 mt-2 bg-white">
              <input
                type="text"
                className="outline-none"
                onChange={(e) =>
                  setUserInfo({ ...userInfo, name: e.target.value })
                }
                placeholder="Enter Your Name"
              />
            </div>
            <div className="border-2 rounded-xl p-1 mt-2 bg-white">
              <input
                type="text"
                className="outline-none"
                onChange={(e) =>
                  setUserInfo({ ...userInfo, email: e.target.value })
                }
                placeholder="Enter Your Email"
              />
            </div>
            <div className="border-2 rounded-xl p-1 mt-2 bg-white">
              <input
                type="text"
                className="outline-none"
                onChange={(e) =>
                  setUserInfo({ ...userInfo, password: e.target.value })
                }
                placeholder="Enter Your Password"
              />
            </div>
            <div className="border-2 rounded-xl p-1 mt-2 bg-white ">
              <input
                className="outline-none"
                type="text"
                placeholder="Confirm Your Password"
              />
            </div>
            <button
              onClick={(e) => submitHandler(e)}
              className="p-2 bg-blue-500 rounded-xl flex justify-center items-center text-center font-bold text-white"
            >
              Submit
            </button>
            <div onClick={(e)=> setHaveUserAccount(true)} className="font-bold underline text-blue-400">
              Have an Account?
            </div>
          </form>
        </div>
      </div>)}

    </div>
  );
}

export default LandingPage;
